...
const trapWidth = 210;
let trapTitleWidth = 150;

const trapTitlesPos = [
    { x: width - width / 4 + trapTitleWidth, y: height / 4 - 10 },
    {
        x: width - width / 4 + trapTitleWidth + 30,
        y: height - height / 4 + 15
    },
    { x: width / 4 - trapTitleWidth - 30, y: height - height / 4 },
    { x: width / 4 - trapTitleWidth - 50, y: height / 4 - 30 }
];
...
function update(source) {
    ...
    Array.from(root.children).forEach((problem, ix) => {
        problem['id'] = ix;
        problem.children.forEach(trap => {
            trap['id'] = ix;
            trap.children.forEach(value => {
                value['id'] = ix;
            });
        });
    });
    ...
    const trap = d3
        ...
        .attr('style', d => {
            ...
            return `left: ${left}px; top: ${top}px; text-align: ${align}; width: ${trapWidth}px;`;
        })
        .append('span')
    ...
    const trapTitle = d3
        ...
        .enter()
        .append('div')
        .attr('class', d => `trap-title node-${d.data.id}`)
        .attr('style', (d, ix) => {
            const stringLen = d.data.name.length;
            if (stringLen < 20) {
                trapTitleWidth = 90;
            ...
            return `left: ${trapTitlesPos[ix].x}px; top: ${trapTitlesPos[ix].y}px; width: ${trapTitleWidth}px;`;
        })
        .text(d => d.data.name);
}
